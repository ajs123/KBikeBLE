...
1. A cable to connect to the bike, via the RJ9 connector on the resistance magnet assembly (right behind the crank shaft on the left side). RJ9 is the standard for handsets on landline phones, so these are easy to get, and it's not hard to tuck under the plastic cover on the bike. Because these are handset cables, you can get coiled versions similar to the one that's in the stock bike. 
   ...
   - A replacement Keiser cable. [They're available](https://www.sportsmith.com/cord-computer-to-pickup/product/10341?serial=1187), they include nice strain releifs for where they pass through the tubing in the bike along with a coiled section to accommodate the handlebar adjustment. They have conductors that should be easy to strip. You can also easilty get the corresponding 2 mm pitch four-pin connectors, which are available in [PCB (solder) form](https://octopart.com/292161-4-te+connectivity-55412967) and in [IDC form](https://octopart.com/173977-4-te+connectivity-46505272) with a [male-to-male connector](https://octopart.com/292156-4-te+connectivity-40070150). NOTE: The author has not verified the conductor colors on these cables. See the notes at the end for how to do that.
